import { CheckCircle2, BadgeDollarSign, CreditCard, Truck } from "lucide-react";
...
export function FeaturesBar() {
  const features = [
    ...
    { icon: <CreditCard className="w-7 h-7" />, title: "ผ่อน 0%", desc: "สูงสุด 4 เดือน" },
    ...
    <section className="w-full relative z-10 -mt-8 md:-mt-12 mb-10">
      <div className="container mx-auto px-4 md:px-8">
        <div className="bg-white rounded-xl shadow-[0_4px_20px_rgb(0,0,0,0.05)] border border-slate-100 p-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-2 lg:gap-y-0 lg:divide-x divide-slate-100">
            {features.map((feature, idx) => (
              <div key={idx} className={`flex items-center gap-4 lg:pl-6 ${idx === 0 ? 'lg:pl-0' : ''}`}>
