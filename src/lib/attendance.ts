...
export type AttendanceSummary = {
  ...
  daysAbsent:      number;
  daysLeave:       number;
  otMinutes:       number;   // raw นาที OT รวม (เพื่อแสดงผล)
  lateMinutes:     number;   // raw นาทีสาย รวม (เพื่อแสดงผล)
  hoursWorked:     number;
  lateBilledHours: number;   // sum ชม.บิลสาย ทีละวัน (ใช้คิดเงิน)
  otBilledHours:   number;   // sum ชม.บิล OT ทีละวัน (ใช้คิดเงิน)
};
...
export async function getAttendanceSummary(period: string): Promise<Record<string, AttendanceSummary>> {
  ...
  for (const r of rows) {
    const s = map[r.employeeId] ?? {
      daysPresent: 0, daysAbsent: 0, daysLeave: 0,
      otMinutes: 0, lateMinutes: 0, hoursWorked: 0,
      lateBilledHours: 0, otBilledHours: 0,
    };
    ...
    // apply grace period PER DAY then accumulate → ถูกต้องสำหรับคิดเงิน
    s.lateBilledHours += minutesToBilledHours(r.lateMinutes);
    s.otBilledHours   += minutesToBilledHours(r.otMinutes);
    map[r.employeeId] = s;
  }
  return map;
}
