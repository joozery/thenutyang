...
import { getTireById, BRAND_LOGOS, CATEGORIES } from '@/lib/tires';
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return { title: `${tire.brand} ${tire.model} ${tire.size} | เดอะนัททายางยนต์` };
}
...
export default async function TireDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const discount = tire.oldPrice
    ? Math.round(((tire.oldPrice - tire.price) / tire.oldPrice) * 100)
    : 0;
    ...
    <div className="bg-slate-50 min-h-screen">
      <div className="container mx-auto px-4 md:px-8 py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-6">
          <Link href="/tires" className="flex items-center gap-1 hover:text-rose-600 transition">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-rose-50 rounded-xl p-4">
              <div className="flex items-end gap-3">
                <span className="text-3xl font-black text-rose-600">฿{tire.price.toLocaleString()}</span>
                <span className="text-sm text-slate-500 mb-1">/ เส้น</span>
              </div>
              {tire.oldPrice && (
                <p className="text-sm text-slate-400 mt-1">
                  ราคาปกติ <span className="line-through">฿{tire.oldPrice.toLocaleString()}</span>
                  <span className="ml-2 text-amber-600 font-bold">ประหยัด ฿{(tire.oldPrice - tire.price).toLocaleString()}</span>
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 text-sm font-medium">
              {tire.inStock ? (
                <>
                  ...
                  <span className="text-green-600">มีสินค้าพร้อมจัดส่ง</span>
                </>
              ) : (
                <>
                  <XCircle className="w-4 h-4 text-slate-400" />
                  <span className="text-slate-500">สินค้าหมด — จองล่วงหน้าได้</span>
                </>
              )}
            </div>
            ...
            <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-slate-50">
                {[
                  ...
                  { label: 'ประเภท', value: tire.specs.type },
                ].map(row => (
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={f.label} className="flex flex-col items-center gap-1.5 bg-white border border-slate-100 rounded-xl p-3 text-center">
                  <div className="text-rose-600">{f.icon}</div>
            ...
            <div className="flex gap-3 pt-2">
              <Link
                ...
                className="flex-1 text-center bg-rose-600 hover:bg-rose-700 text-white font-bold py-3.5 px-6 rounded-xl transition-colors shadow-lg shadow-rose-200 text-sm"
              >
                จองเลย / ขอใบเสนอราคา
              </Link>
              <Link
                href="https://line.me/ti/p/~@thenutyang"
                target="_blank"
            ...
            <p className="text-xs text-slate-400 text-center">
              ราคาต่อเส้น · ติดตั้ง ณ ร้าน · ผ่อน 0% สูงสุด 10 เดือน
            </p>
          </div>
        </div>
      </div>
